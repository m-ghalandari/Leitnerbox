...
app.delete("/api/delete/:id", async (req, res) => {
  ...
  const sqlDelete = "DELETE FROM flashcards WHERE id = ?;";
...
app.put("/api/editCard", async (req, res) => {
  ...
  const sqlEdit =
    "UPDATE flashcards SET box=?, level=?, front=?, back=?, example=? WHERE id=?;";

  try {
    const [rows, fields] = await db.query(sqlEdit, [
      box,
      level,
      front,
      back,
...
app.put("/api/increaseLevels", async (req, res) => {
  ...
  for (const card of cards) {
    promises.push(
      db.query("UPDATE flashcards SET level = ? WHERE id = ?", [
        card.level + 1,
        card.id,
      ])
    );
...
async function readAll() {
  ...
  for (const card of data.flashcards) {
    promises.push(
      db.query(
        "INSERT INTO flashcards (id,box, level, front, back, example) VALUES (?,?,?,?,?,?);",
        [card.id, card.box, card.level, card.front, card.back, card.example]
      )
    );
  }
  try {
